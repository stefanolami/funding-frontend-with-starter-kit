                ...
                <div class="flex flex-row justify-between flex-wrap">
                    ${map(this.countries.filter(country => country.name.toLowerCase().includes(this.searchInput)), (country, index) => {
                        if (this.countries.filter(country => country.name.toLowerCase().includes(this.searchInput)).length == 1) {
                            ...
                            return html`
                                <div class="z-50 flex flex-row items-center w-full h-[26px] mt-[6px] pl-3 border-[1px] border-primary rounded-full bg-white cursor-pointer select-none">                       
                                ...
                                <div class="who-we-are-card my-[6px] w-full hidden">
                                    <div class="flex items-center h-[26px] pl-5 rounded-full bg-primary/60 font-unna-700 text-sm text-white select-none">${country.name}</div>
                                    <div class="w-[90%] mx-auto">
                                        <p class="who-we-are-card-p">Education Expenditure (World Bank):</p>
                                        <span class="who-we-are-card-span">${country.education}</span>
                            ...
                            return html`
                                <div id="${this.formatCountry(country.name)}-card" class="who-we-are-card my-[6px] w-full hidden">
                                    <div class="flex items-center h-[26px] pl-5 rounded-full bg-primary/60 font-unna-700 text-sm text-white select-none">${country.name}</div>
                                    <div class="w-[90%] mx-auto">
                                        <p class="who-we-are-card-p">Education Expenditure (World Bank):</p>
                                        <span class="who-we-are-card-span">${country.education}</span>
                                ...
                                <div @click=${() => this.handleClick(this.formatCountry(country.name), index)} id="${this.formatCountry(country.name)}-btn" class="h-[26px] w-[49%] my-[6px] pl-5 pt-[1px] bg-primary rounded-full flex items-center text-white font-unna-700 text-sm">${country.name}</div>
                                <div id="${this.formatCountry(country.name)}-btn-placeholder" class="hidden h-[26px] w-[49%] my-[6px] pl-5 pt-[1px] bg-white rounded-full text-white font-unna-700 text-sm"></div>
                            `
                        } else {
                            return html`
                                <div @click=${() => this.handleClick(this.formatCountry(country.name), index)} id="${this.formatCountry(country.name)}-btn" class="h-[26px] w-[49%] my-[6px] pl-5 pt-[1px] bg-primary rounded-full flex items-center text-white font-unna-700 text-sm">${country.name}</div>
